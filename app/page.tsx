...
export default function ERPDashboard() {
  ...
  // Dashboard Metric States matching Streamlit logic
  ...
  useEffect(() => {
    async function fetchStreamlitParityMetrics() {
      ...
      // 3. Current Fiscal Month start date calculation
      const now = new Date();
      const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();

      // 4. Fetch all trips for the month (matching Streamlit's cumulative monthly calculation)
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-4">
              <div className="flex justify-between items-center border-b pb-2">
                <h3 className="text-base font-extrabold uppercase text-slate-900">
                  Operations Summary ({new Date().toLocaleString('default', { month: 'long', year: 'numeric' }).toUpperCase()})
                </h3>
              </div>
              ...
              <div className="border border-red-100 bg-red-50 rounded-lg p-4 shadow-sm max-w-sm">
                <p className="text-xs font-bold text-red-700 uppercase mb-2">Month Freight Generated</p>
