    ...
    if (drivers) {
      drivers.forEach(d => {
        if (d.license_expiry_date && new Date(d.license_expiry_date) <= thirtyDaysFromNow) {
          alerts.push({ name: `${d.driver_code} - ${d.full_name}`, doc: "Driving License", date: d.license_expiry_date });
        }
      });
    }
          ...
          <div className="flex items-center gap-3 sm:gap-4 shrink-0">
            ...
            {isAuthenticated ? (
              ...
              <button 
                ...
                className="flex items-center gap-2 text-xs sm:text-sm font-black text-white hover:bg-[#e04f00] bg-[#FF5A00] px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl transition-all shadow-sm active:scale-95"
              >
                ...
                {pendingDriverCount > 0 && (
                  <div className="bg-amber-50 border-l-4 border-amber-500 rounded-2xl shadow-sm p-4 sm:p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 animate-in slide-in-from-top-4">
                    ...
                    <button 
                      ...
                      className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-xs font-black rounded-xl transition-colors shadow-sm whitespace-nowrap"
                    >
                      Review Queue &rarr;
                    </button>
                  </div>
