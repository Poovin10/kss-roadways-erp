...
export function SetupModule() {
  ...
  const handleEditDriver = (d: any) => {
    setEditDriverId(d.driver_id); setDriverName(d.full_name); setMobileNo(d.phone_number || "");
    setLicenseNo(d.license_number || ""); setExpiryDate(d.license_expiry_date || ""); setIsActiveDriver(d.is_active);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleEditTruck = (t: any) => {
    setEditTruckId(t.id); setTruckNo(t.vehicle_number); setVariant(t.truck_type || "Bulker (16-Wheel)");
    setCapacity(`${t.carrying_capacity_tons}.0 MT`); window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const handleEditSlab = (s: any) => {
    ...
    setCType(s.cargo_type); setCap(s.capacity_tons); setFRate(s.freight_rate_per_ton); setAvgKms(s.standard_km || "");
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const handleEditBata = (b: any) => {
    ...
    setCType(b.cargo_type); setBataCap(b.capacity_tons); setBataAmt(b.standard_bata_inr);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  useEffect(() => {
    if (selectedTruckForCompliance) {
      setFcExp(selectedTruckForCompliance.fc_expiry_date || ""); setInsExp(selectedTruckForCompliance.insurance_expiry_date || "");
      setQtaxExp(selectedTruckForCompliance.qtax_expiry_date || ""); setPucExp(selectedTruckForCompliance.puc_expiry_date || "");
      setNpExp(selectedTruckForCompliance.np_expiry_date || ""); setSpExp(selectedTruckForCompliance.state_permit_expiry_date || "");
  ...
  const handleSaveTruckCompliance = (e: React.FormEvent) => {
    ...
    triggerModal("Update Compliance", `Save document expiry dates for ${selectedTruckForCompliance.vehicle_number}?`, false, "Save Dates", async () => {
      ...
      const { error } = await supabase.from('trucks').update({
        fc_expiry_date: fcExp || null, insurance_expiry_date: insExp || null, qtax_expiry_date: qtaxExp || null,
        puc_expiry_date: pucExp || null, np_expiry_date: npExp || null, state_permit_expiry_date: spExp || null, tank_cert_expiry_date: tankExp || null
      }).eq('id', selectedTruckForCompliance.id);

      if (error) alert("Error updating compliance: " + error.message);
      else { alert("Truck compliance updated successfully!"); fetchData(); }
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (currentUsername !== "superadmin" && currentUserRole !== "SUPERADMIN") return alert("Unauthorized: Only Superadmins can perform this action.");
    ...
    triggerModal("Create User", `Create new ${newRole} account for ${newUsername.trim().toLowerCase()}?`, false, "Create User", async () => {
      ...
      try {
        ...
        if (!res.ok) alert("Failed to create user: " + data.error);
        else { setNewUsername(""); setNewPassword(""); setNewRole("VIEWER"); fetchData(); }
      } catch (err) { alert("Network error occurred while creating user."); }
      setIsProcessing(false); closeModal();
    });
  };
  ...
  const handleDeleteUser = (username: string) => {
    ...
    if (currentUsername !== "superadmin" && currentUserRole !== "SUPERADMIN") return alert("Unauthorized: Only Superadmins can revoke access.");

    triggerModal("Delete User", `Are you sure you want to revoke access for ${username}?`, true, "Delete", async () => {
      setIsProcessing(true); await supabase.from('app_users').delete().eq('username', username);
  ...
  const handleResetDriverPin = async (driverId: number, driverName: string) => {
    ...
    if (!newPin) return;
    if (newPin.length !== 4 || isNaN(Number(newPin))) return alert("Invalid PIN. It must be exactly 4 digits.");
    setIsProcessing(true);
    const { error } = await supabase.from('drivers').update({ pin: newPin }).eq('driver_id', driverId);
    if (error) alert("Error resetting PIN: " + error.message);
    else { alert(`PIN for ${driverName} successfully reset to ${newPin}`); fetchData(); }
  ...
  const handleSaveTruck = (e: React.FormEvent) => {
    ...
    triggerModal(isUpdate ? "Update Truck" : "Add Truck", isUpdate ? `Update details for ${truckNo.toUpperCase()}?` : `Register ${truckNo.toUpperCase()} to the fleet?`, false, "Save Truck", async () => {
      ...
      if (isUpdate) {
        const { error } = await supabase.from('trucks').update(payload).eq('id', editTruckId);
  ...
  const handleSaveDriver = async (e: React.FormEvent) => {
    ...
    } catch (err: any) { alert("Error handling driver code: " + err.message); setIsProcessing(false); }
  };
  ...
  const handleSaveSlab = (e: React.FormEvent) => {
    ...
    triggerModal(isUpdate ? "Update Freight Slab" : "Add Freight Slab", isUpdate ? `Update rate for ${finalSrc.toUpperCase()} to ${finalDest.toUpperCase()}?` : `Lock in ₹${fRate}/MT for ${finalSrc.toUpperCase()} to ${finalDest.toUpperCase()}?`, false, "Save Slab", async () => {
      setIsProcessing(true);
      const payload = { origin: finalSrc.toUpperCase().trim(), destination_name: finalDest.toUpperCase().trim(), cargo_type: cType, capacity_tons: cap, standard_km: Number(avgKms), freight_rate_per_ton: Number(fRate), is_active: true };
      let error: any;
      if (isUpdate) {
  ...
  const handleSaveBata = (e: React.FormEvent) => {
    ...
    triggerModal(isUpdate ? "Update Bata Master" : "Add Bata Master", isUpdate ? `Update Bata for ${finalSrc.toUpperCase()} to ${finalDest.toUpperCase()}?` : `Set ₹${bataAmt} default Bata for ${finalSrc.toUpperCase()} to ${finalDest.toUpperCase()}?`, false, "Save Bata", async () => {
      setIsProcessing(true);
      const payload = { origin: finalSrc.toUpperCase().trim(), destination_name: finalDest.toUpperCase().trim(), cargo_type: cType, capacity_tons: bataCap, standard_bata_inr: Number(bataAmt) };
      let error: any;
      if (isUpdate) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-wrap gap-2 border-b border-[#272B36] pb-4">
        {[
          { label: "Trucks", icon: "🚛" }, { label: "Drivers", icon: "👨‍✈️" }, { label: "Freight Slabs", icon: "🛣️" },
          { label: "Bata", icon: "💰" }, { label: "🚚 Truck Compliance", icon: "🛡️" }, { label: "🔐 User Control", icon: "⚙️" }
        ].map((tab) => (
          <button
      ...
      <div className="bg-[#161922] border border-[#272B36] rounded-2xl p-6 sm:p-8 shadow-xl max-w-6xl mx-auto">
        ...
        {sTab === "Trucks" && (
          <>
            <div className="flex justify-between items-center border-b border-[#272B36] pb-3 mb-6">
              <h3 className="text-sm font-black text-white uppercase tracking-wide">{editTruckId ? "Edit Existing Truck" : "Add New Truck"}</h3>
            ...
            <div className="border border-[#272B36] rounded-xl overflow-hidden w-full">
              <TableToolbar title="Registered Fleet" searchQuery={truckSearch} setSearchQuery={setTruckSearch} exportData={exportTrucks} exportFilename="Fleet_Trucks" />
              <div className="overflow-x-auto max-h-80 overflow-y-auto">
                <table className="min-w-full text-xs text-left whitespace-nowrap">
                  <thead className="bg-[#0F1117] text-slate-400 uppercase font-bold sticky top-0">
                    <tr><th className="p-3">Truck No</th><th className="p-3">Variant</th><th className="p-3">Capacity</th><th className="p-3">Status</th></tr>
                  </thead>
                  <tbody className="divide-y divide-[#272B36] bg-[#161922]">
                    {filteredTrucks.map(t => (
                      <tr key={t.id} onClick={() => handleEditTruck(t)} className={`cursor-pointer transition-colors ${editTruckId === t.id ? 'bg-[#FF5A00]/10 border-l-2 border-l-[#FF5A00]' : 'hover:bg-[#1E222D] border-l-2 border-transparent'}`}>
                        <td className="p-3 font-bold text-[#FF5A00]">{t.vehicle_number}</td>
                        <td className="p-3 text-white font-semibold">{t.truck_type}</td>
                        <td className="p-3 text-slate-300">{t.carrying_capacity_tons} MT</td>
                        <td className="p-3"><span className="px-2 py-1 rounded bg-[#0F1117] border border-[#272B36] text-[9px] font-black uppercase text-slate-300">{t.current_status || "AVAILABLE"}</span></td>
                      </tr>
                    ))}
        ...
        {sTab === "Drivers" && (
          <>
            <div className="flex justify-between items-center border-b border-[#272B36] pb-3 mb-6">
              <h3 className="text-sm font-black text-white uppercase tracking-wide">{editDriverId ? "Edit Existing Driver Profile" : "Add New Driver"}</h3>
            ...
            <form onSubmit={handleSaveDriver} className="space-y-4 mb-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Driver Code</label><input type="text" value={editDriverId ? "LOCKED IN EDIT MODE" : `DRV-[Auto-Generated]`} disabled className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-black uppercase bg-[#0F1117] text-[#FF5A00] cursor-not-allowed" /></div>
                <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Full Name *</label><input type="text" maxLength={40} value={driverName} onChange={e=>setDriverName(e.target.value.toUpperCase())} placeholder="e.g. ANEESH CR" className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-bold uppercase bg-[#0F1117] text-white focus:border-[#FF5A00]" required /></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-start">
                <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Mobile Number</label><input type="tel" maxLength={15} value={mobileNo} onChange={e=>setMobileNo(e.target.value)} placeholder="e.g. 9876543210" className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-semibold bg-[#0F1117] text-white focus:border-[#FF5A00]" /></div>
                <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">License Number</label><input type="text" maxLength={30} value={licenseNo} onChange={e=>setLicenseNo(e.target.value.toUpperCase())} placeholder="e.g. KL123456789" className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-semibold uppercase bg-[#0F1117] text-white focus:border-[#FF5A00]" /></div>
                <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">License Expiry</label><input type="date" value={expiryDate} onChange={e=>setExpiryDate(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-semibold bg-[#0F1117] text-white focus:border-[#FF5A00]" /></div>
            ...
            <div className="border border-[#272B36] rounded-xl overflow-hidden w-full">
              <TableToolbar title="Registered Drivers" searchQuery={driverSearch} setSearchQuery={setDriverSearch} exportData={exportDrivers} exportFilename="Fleet_Drivers" />
              <div className="overflow-x-auto max-h-80 overflow-y-auto">
                <table className="min-w-full text-xs text-left whitespace-nowrap">
                  <thead className="bg-[#0F1117] text-slate-400 uppercase font-bold sticky top-0"><tr><th className="p-3">Code</th><th className="p-3">Full Name</th><th className="p-3">Phone</th><th className="p-3">License Info</th><th className="p-3">Status</th></tr></thead>
                  <tbody className="divide-y divide-[#272B36] bg-[#161922]">
                    ...
                    {filteredDrivers.length === 0 && <tr><td colSpan={5} className="p-4 text-center text-slate-400">No drivers match search.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
        ...
        {sTab === "Freight Slabs" && (
          <>
            <div className="flex justify-between items-center border-b border-[#272B36] pb-3 mb-6">
              <h3 className="text-sm font-black text-white uppercase tracking-wide">{editSlabId ? "Edit Existing Freight Slab" : "Add New Freight Slab"}</h3>
            ...
            <form onSubmit={handleSaveSlab} className="grid grid-cols-1 md:grid-cols-7 gap-4 items-start mb-8">
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Source</label><select value={src} onChange={e=>setSrc(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-bold bg-[#0F1117] text-white">{originOptions.map(s=><option key={s} value={s}>{s}</option>)}<option value="CUSTOM">-- TYPE NEW --</option></select>{src === "CUSTOM" && <input type="text" maxLength={40} value={customSrc} onChange={e=>setCustomSrc(e.target.value.toUpperCase())} placeholder="New Source" className="w-full text-sm p-3 mt-2 rounded-xl border border-[#272B36] outline-none uppercase font-bold bg-[#0F1117] text-white" required />}</div>
              <div className="md:col-span-2"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Destination *</label><select value={dest} onChange={e=>setDest(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-bold bg-[#0F1117] text-white uppercase" required><option value="">-- SELECT DEST --</option>{destOptions.map(d=><option key={d} value={d}>{d}</option>)}<option value="CUSTOM">-- TYPE NEW --</option></select>{dest === "CUSTOM" && <input type="text" maxLength={40} value={customDest} onChange={e=>setCustomDest(e.target.value.toUpperCase())} placeholder="New Destination" className="w-full text-sm p-3 mt-2 rounded-xl border border-[#272B36] outline-none uppercase font-bold bg-[#0F1117] text-white" required />}</div>
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Cargo</label><select value={cType} onChange={e => { setCType(e.target.value); if (e.target.value === "BAG") { setCap("25/30"); } if (e.target.value === "BULK" && cap !== "35") setCap("25/30"); }} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none bg-[#0F1117] text-white"><option value="BULK">BULK</option><option value="BAG">BAG</option></select></div>
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Cap (MT)</label><select value={cap} onChange={e=>setCap(e.target.value)} disabled={cType === "BAG"} className={`w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none bg-[#0F1117] ${cType === "BAG" ? "text-slate-500 cursor-not-allowed" : "text-white"}`}>{cType === "BAG" ? <option value="25/30">25/30 MT</option> : <><option value="25/30">25/30 MT</option><option value="35">35 MT</option></>}</select></div>
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Rate(₹)</label><input type="number" min="1" value={fRate} onChange={e=>setFRate(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-black text-emerald-400 bg-[#0F1117]" required /></div>
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Avg KMs</label><input type="number" min="1" value={avgKms} onChange={e=>setAvgKms(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-black text-sky-400 bg-[#0F1117]" required /></div>
            ...
            <div className="border border-[#272B36] rounded-xl overflow-hidden w-full">
              <TableToolbar title="Freight Slabs Master" searchQuery={slabSearch} setSearchQuery={setSlabSearch} exportData={exportSlabs} exportFilename="Freight_Slabs" />
              <div className="overflow-x-auto max-h-80 overflow-y-auto">
                <table className="min-w-full text-xs text-left whitespace-nowrap">
                  <thead className="bg-[#0F1117] text-slate-400 uppercase font-bold sticky top-0"><tr><th className="p-3">Route</th><th className="p-3">Type</th><th className="p-3 text-center">Avg KMs</th><th className="p-3 text-right">Rate/MT</th></tr></thead>
                  <tbody className="divide-y divide-[#272B36] bg-[#161922]">
                    {filteredSlabs.map(s => (
                      <tr key={s.destination_id} onClick={() => handleEditSlab(s)} className={`cursor-pointer transition-colors ${editSlabId === s.destination_id ? 'bg-[#FF5A00]/10 border-l-2 border-l-[#FF5A00]' : 'hover:bg-[#1E222D] border-l-2 border-transparent'}`}>
                    ...
                    {filteredSlabs.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-slate-400">No slabs match search.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
        ...
        {sTab === "Bata" && (
          <>
            <div className="flex justify-between items-center border-b border-[#272B36] pb-3 mb-6">
              <h3 className="text-sm font-black text-white uppercase tracking-wide">{editBataId ? "Edit Existing Bata Rule" : "Add Bata Rule"}</h3>
            ...
            <form onSubmit={handleSaveBata} className="grid grid-cols-1 md:grid-cols-6 gap-4 items-start mb-8">
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Source</label><select value={src} onChange={e=>setSrc(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-bold bg-[#0F1117] text-white">{originOptions.map(s=><option key={s} value={s}>{s}</option>)}<option value="CUSTOM">-- TYPE NEW --</option></select>{src === "CUSTOM" && <input type="text" maxLength={40} value={customSrc} onChange={e=>setCustomSrc(e.target.value.toUpperCase())} placeholder="New Source" className="w-full text-sm p-3 mt-2 rounded-xl border border-[#272B36] outline-none uppercase font-bold bg-[#0F1117] text-white" required />}</div>
              <div className="md:col-span-2"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Destination *</label><select value={dest} onChange={e=>setDest(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none uppercase font-bold bg-[#0F1117] text-white" required><option value="">-- SELECT DEST --</option>{destOptions.map(d=><option key={d} value={d}>{d}</option>)}<option value="CUSTOM">-- TYPE NEW --</option></select>{dest === "CUSTOM" && <input type="text" maxLength={40} value={customDest} onChange={e=>setCustomDest(e.target.value.toUpperCase())} placeholder="New Destination" className="w-full text-sm p-3 mt-2 rounded-xl border border-[#272B36] outline-none uppercase font-bold bg-[#0F1117] text-white" required />}</div>
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Cargo</label><select value={cType} onChange={e => { setCType(e.target.value); if (e.target.value === "BAG" && bataCap === "35") setBataCap("30"); }} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none bg-[#0F1117] text-white"><option value="BULK">BULK</option><option value="BAG">BAG</option></select></div>
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Cap (MT)</label><select value={bataCap} onChange={e=>setBataCap(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none bg-[#0F1117] text-white">{cType === "BAG" ? <><option value="25">25 MT</option><option value="30">30 MT</option></> : <><option value="25">25 MT</option><option value="30">30 MT</option><option value="35">35 MT</option></>}</select></div>
              <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Bata(₹)</label><input type="number" min="1" value={bataAmt} onChange={e=>setBataAmt(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none font-black text-[#FF5A00] bg-[#0F1117]" required /></div>
            ...
            <div className="border border-[#272B36] rounded-xl overflow-hidden w-full">
              <TableToolbar title="Driver Bata Master" searchQuery={bataSearch} setSearchQuery={setBataSearch} exportData={exportBata} exportFilename="Bata_Master" />
              <div className="overflow-x-auto max-h-80 overflow-y-auto">
                <table className="min-w-full text-xs text-left whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-[#272B36] bg-[#161922]">
                    {filteredBata.map(b => (
                      <tr key={b.bata_rule_id} onClick={() => handleEditBata(b)} className={`cursor-pointer transition-colors ${editBataId === b.bata_rule_id ? 'bg-[#FF5A00]/10 border-l-2 border-l-[#FF5A00]' : 'hover:bg-[#1E222D] border-l-2 border-transparent'}`}>
                    ...
                    {filteredBata.length === 0 && <tr><td colSpan={3} className="p-4 text-center text-slate-400">No bata rules match search.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              <select
                ...
                onChange={e => { const found = trucksList.find(t => String(t.id) === e.target.value); setSelectedTruckForCompliance(found || null); }}
              ...
              >
                ...
                {trucksList.map(t => <option key={t.id} value={t.id}>{t.vehicle_number} [{t.truck_type}]</option>)}
              </select>
            </div>

            {selectedTruckForCompliance && (
              <form onSubmit={handleSaveTruckCompliance} className="space-y-6 animate-in slide-in-from-bottom-4 mb-8">
                <div className="bg-[#0F1117] p-4 rounded-xl border border-[#272B36] flex justify-between items-center">
                  <div><p className="text-xs font-bold text-slate-400 uppercase">Selected Vehicle</p><p className="text-base font-black text-[#FF5A00]">{selectedTruckForCompliance.vehicle_number}</p></div>
                  <div><p className="text-xs font-bold text-slate-400 uppercase">Variant</p><p className="text-sm font-bold text-white">{selectedTruckForCompliance.truck_type}</p></div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">FC Test Expiry</label><input type="date" value={fcExp} onChange={e => setFcExp(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] font-semibold bg-[#0F1117] text-white outline-none focus:border-[#FF5A00]" /></div>
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Insurance Expiry</label><input type="date" value={insExp} onChange={e => setInsExp(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] font-semibold bg-[#0F1117] text-white outline-none focus:border-[#FF5A00]" /></div>
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Quarterly Tax (Q-Tax) Expiry</label><input type="date" value={qtaxExp} onChange={e => setQtaxExp(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] font-semibold bg-[#0F1117] text-white outline-none focus:border-[#FF5A00]" /></div>
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">PUC Expiry</label><input type="date" value={pucExp} onChange={e => setPucExp(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] font-semibold bg-[#0F1117] text-white outline-none focus:border-[#FF5A00]" /></div>
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">National Permit (NP) Expiry</label><input type="date" value={npExp} onChange={e => setNpExp(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] font-semibold bg-[#0F1117] text-white outline-none focus:border-[#FF5A00]" /></div>
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">State Permit Expiry</label><input type="date" value={spExp} onChange={e => setSpExp(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] font-semibold bg-[#0F1117] text-white outline-none focus:border-[#FF5A00]" /></div>
                ...
                <div className="pt-4 border-t border-[#272B36] flex justify-end">
                  <button type="submit" disabled={isProcessing} className="px-8 py-3 bg-[#FF5A00] hover:bg-[#e04f00] text-white font-black text-sm rounded-xl transition-all shadow-lg active:scale-95">Save Compliance Dates</button>
                </div>
              </form>
            )}

            <div className="border border-[#272B36] rounded-xl overflow-hidden w-full">
              <TableToolbar title="Fleet Compliance Overview" searchQuery={complianceSearch} setSearchQuery={setComplianceSearch} exportData={exportComp} exportFilename="Fleet_Compliance_Report" />
              <div className="overflow-x-auto max-h-80 overflow-y-auto">
                <table className="min-w-full text-xs text-left whitespace-nowrap">
                  <thead className="bg-[#0F1117] text-slate-400 uppercase font-bold sticky top-0">
                    <tr><th className="p-3">Truck No</th><th className="p-3">FC Expiry</th><th className="p-3">Insurance</th><th className="p-3">Q-Tax</th><th className="p-3">PUC</th><th className="p-3">NP Expiry</th></tr>
                  </thead>
                  <tbody className="divide-y divide-[#272B36] bg-[#161922]">
                    {filteredComp.map(t => (
                      <tr key={t.id} onClick={() => { setSelectedTruckForCompliance(t); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`cursor-pointer transition-colors ${selectedTruckForCompliance?.id === t.id ? 'bg-[#FF5A00]/10 border-l-2 border-l-[#FF5A00]' : 'hover:bg-[#1E222D] border-l-2 border-transparent'}`}>
                        <td className="p-3 font-bold text-[#FF5A00]">{t.vehicle_number}</td>
                    ...
                    {filteredComp.length === 0 && <tr><td colSpan={6} className="p-4 text-center text-slate-400">No trucks match search.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
        ...
        {sTab === "🔐 User Control" && (
          <>
            {currentUsername !== "superadmin" && currentUserRole !== "SUPERADMIN" ? (
              <div className="p-12 text-center bg-rose-950/20 border border-rose-900/50 rounded-2xl">
                <p className="text-3xl mb-4">🛑</p>
                <p className="text-xl font-black text-rose-400">Security Clearance Required</p>
                <p className="text-sm text-rose-300 mt-2">This module is strictly restricted to the Super Admin account.</p>
              </div>
            ) : (
              <div>
                ...
                <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-8">
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Username *</label><input type="text" maxLength={20} value={newUsername} onChange={e => setNewUsername(e.target.value.toLowerCase().replace(/\s/g, ''))} placeholder="e.g. manager2" className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none bg-[#0F1117] text-white font-bold focus:border-[#FF5A00]" required /></div>
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Password *</label><input type="password" minLength={6} value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="••••••••" className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none bg-[#0F1117] text-white font-bold focus:border-[#FF5A00]" required /></div>
                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Role *</label><select value={newRole} onChange={e => setNewRole(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] outline-none bg-[#0F1117] text-white font-bold"><option value="VIEWER">VIEWER</option><option value="ADMIN">ADMIN</option></select></div>
                  <button type="submit" disabled={isProcessing} className="w-full py-3 bg-[#FF5A00] text-white font-black rounded-xl hover:bg-[#e04f00] transition-colors shadow-lg active:scale-95">Add User</button>
                </form>
                ...
                <div className="overflow-x-auto border border-[#272B36] rounded-xl w-full max-h-80 mb-8">
                  <table className="min-w-full text-xs text-left whitespace-nowrap">
                    <thead className="bg-[#0F1117] text-slate-400 uppercase font-bold sticky top-0"><tr><th className="p-3">Username</th><th className="p-3">Role</th><th className="p-3 text-center">Action</th></tr></thead>
                    <tbody className="divide-y divide-[#272B36] bg-[#161922]">
                      {systemUsers.map(u => (
                        <tr key={u.user_id} className="hover:bg-[#1E222D]">
                          ...
                          <td className="p-3 text-center">{u.username !== 'superadmin' ? (<button onClick={() => handleDeleteUser(u.username)} className="px-3 py-1 bg-rose-950/50 text-rose-400 font-bold rounded-lg hover:bg-rose-900 border border-rose-800/50 transition-colors">Revoke Access</button>) : (<span className="text-[10px] text-slate-500 italic">Protected Core Admin</span>)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="overflow-x-auto border border-[#272B36] rounded-xl w-full max-h-80">
                  <table className="min-w-full text-xs text-left whitespace-nowrap">
                    <thead className="bg-[#0F1117] text-slate-400 uppercase font-bold sticky top-0"><tr><th className="p-3">Driver Code</th><th className="p-3">Full Name</th><th className="p-3 text-center">App Access</th></tr></thead>
                    <tbody className="divide-y divide-[#272B36] bg-[#161922]">
                      {driversList.map(d => (
                        <tr key={d.driver_id} className="hover:bg-[#1E222D]"><td className="p-3 font-bold text-[#FF5A00]">{d.driver_code}</td><td className="p-3 text-white font-semibold">{d.full_name}</td><td className="p-3 text-center"><button onClick={() => handleResetDriverPin(d.driver_id, d.full_name)} className="px-4 py-1.5 bg-amber-950/40 text-amber-500 font-bold rounded-lg hover:bg-amber-900 border border-amber-800/50 transition-colors">Reset PIN</button></td></tr>
                      ))}
                      {driversList.length === 0 && <tr><td colSpan={3} className="p-4 text-center text-slate-400">No drivers available.</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
