...
export function WorkshopModule() {
 ...
 <div>
 <h2 className="text-xl font-black text-white uppercase tracking-tight">Workshop & Inventory</h2>
 <p className="text-xs text-slate-400 mt-0.5">Manage tyre lifecycles, retreading, spares, and service billing.</p>
 </div>
 <div className="flex flex-wrap gap-2">
 {["Tyre Management", "Spares & Service Bills"].map((tab) => (
 <button key={tab} onClick={() => setWTab(tab)} className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${wTab === tab ? "bg-[#FF5A00] text-white shadow-lg shadow-[#FF5A00]/20" : "bg-[#161922] text-slate-400 hover:text-white hover:bg-[#1E222D] border border-[#272B36]"}`}>{tab}</button>
 ))}
 </div>
 </div>
 ...
 <h3 className="text-sm font-black text-white uppercase border-b border-[#272B36] pb-3 mb-4 flex items-center gap-2"><span></span> Add New Tyre to Database</h3>
 <form onSubmit={handleRegisterTyre} className="space-y-4">
 <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
 <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Destination *</label><select value={regMode} onChange={e => setRegMode(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] text-white focus:border-[#FF5A00] outline-none font-bold"><option value="IN_STORE">Add to Store / Inventory</option><option value="MOUNTED">Mount Directly to Truck</option></select></div>
 <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Serial Number *</label><input type="text" maxLength={30} value={serialNo} onChange={e => setSerialNo(e.target.value.toUpperCase())} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] text-white focus:border-[#FF5A00] outline-none font-bold uppercase" required /></div>
 <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Brand / Model</label><input type="text" maxLength={40} value={brand} onChange={e => setBrand(e.target.value.toUpperCase())} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] text-white focus:border-[#FF5A00] outline-none font-semibold uppercase" /></div>
 <div className="md:col-span-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Initial NSD (MM)</label><input type="number" step="0.1" min="0" max="30" value={nsdMm} onChange={e => setNsdMm(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] text-white focus:border-[#FF5A00] outline-none font-bold" /></div>
 </div>
 {regMode === "MOUNTED" && (
 <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-[#0F1117] border border-[#FF5A00]/20 rounded-xl">
 <div><label className="block text-[10px] font-bold text-[#FF5A00] uppercase mb-1">Select Truck *</label><select value={truckId} onChange={e => setTruckId(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#12141C] text-white focus:border-[#FF5A00] outline-none font-bold"><option value="">-- SELECT TRUCK --</option>{vehicles.map(v => <option key={v.id} value={String(v.id)}>{v.vehicle_number}</option>)}</select></div>
 <div><label className="block text-[10px] font-bold text-[#FF5A00] uppercase mb-1">Position</label><select value={position} onChange={e => setPosition(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#12141C] text-white focus:border-[#FF5A00] outline-none font-semibold"><option value="FRONT_LEFT">FRONT_LEFT</option><option value="FRONT_RIGHT">FRONT_RIGHT</option><option value="DRIVE">DRIVE AXLE</option><option value="STEPNEY">STEPNEY</option></select></div>
 ...
 <div className="flex justify-end pt-2"><button type="submit" disabled={isProcessing} className="px-8 py-3 bg-[#FF5A00] hover:bg-[#e04f00] text-white font-black text-sm rounded-xl transition-all shadow-lg shadow-[#FF5A00]/20 active:scale-95 disabled:bg-slate-700">Save Tyre Data</button></div>
 </form>
 </div>
 ...
 <td className="px-5 py-3.5 font-black text-white">{t.trucks?.vehicle_number || "UNKNOWN"}</td>
 <td className="px-5 py-3.5 font-mono font-bold text-white">{t.serial_number} <br/><span className="font-sans font-semibold text-[10px] text-slate-400">{t.brand_model}</span></td>
 <td className="px-5 py-3.5 text-slate-300 font-bold">{t.placement_position}</td>
 <td className="px-5 py-3.5 text-slate-400">{formatDate(t.mounted_date)}</td>
 <td className="px-5 py-3.5 font-black text-emerald-400">{t.total_km_run || 0} km</td>
 ...
 {filteredMounted.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-slate-500 font-medium">No mounted tyres found.</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
 ...
 {filteredStore.map(t => (
 <tr key={t.tyre_id} className="hover:bg-[#1E222D]">
 <td className="px-5 py-3.5"><span className={`px-2 py-1 rounded text-[10px] font-black ${t.tyre_status === 'RETREADING' ? 'bg-amber-950/50 text-amber-500 border border-amber-900' : 'bg-[#0F1117] text-white border border-[#272B36]'}`}>{t.tyre_status || 'IN_STORE'}</span></td>
 <td className="px-5 py-3.5 font-mono font-bold text-white">{t.serial_number} <br/><span className="font-sans font-semibold text-[10px] text-slate-400">{t.brand_model}</span></td>
 <td className="px-5 py-3.5 text-slate-300 font-bold">{t.tyre_condition}</td>
 <td className="px-5 py-3.5 font-black text-sky-400">{t.total_km_run || 0} km</td>
 ...
 <button onClick={() => { setNextState("SCRAPPED"); setActionModal({ isOpen: true, tyre: t, mode: "SCRAP_FROM_STORE" }); }} className="px-3 py-1.5 bg-[#0F1117] hover:bg-[#272B36] text-slate-400 font-bold rounded-lg border border-[#272B36]">Dispose</button>
 </div>
 )}
 </td>
 </tr>
 ))}
 {filteredStore.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-slate-500 font-medium">Inventory is empty.</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
 ...
 <table className="min-w-full divide-y divide-[#272B36] text-xs text-left whitespace-nowrap">
 <thead className="bg-[#0F1117] sticky top-0 z-10"><tr className="font-bold text-slate-400 uppercase tracking-wider text-[10px]"><th className="px-5 py-3.5">Serial & Brand</th><th className="px-5 py-3.5">Status</th><th className="px-5 py-3.5">Total Lifetime Run (KM)</th></tr></thead>
 <tbody className="divide-y divide-[#272B36] bg-[#161922]">
 {filteredScrap.map(t => (
 <tr key={t.tyre_id} className="hover:bg-[#1E222D]">
 <td className="px-5 py-3.5 font-mono font-bold text-white">{t.serial_number} <br/><span className="font-sans font-semibold text-[10px] text-slate-400">{t.brand_model}</span></td>
 <td className="px-5 py-3.5"><span className="px-2 py-1 rounded text-[10px] font-black bg-rose-950/40 text-rose-500 border border-rose-900/50">{t.tyre_status}</span></td>
 <td className="px-5 py-3.5 font-black text-slate-300">{t.total_km_run || 0} km</td>
 </tr>
 ))}
 {filteredScrap.length === 0 && <tr><td colSpan={3} className="p-8 text-center text-slate-500 font-medium">No scrapped tyres.</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
 ...
 <h3 className="text-sm font-black text-white uppercase border-b border-[#272B36] pb-3 mb-6">Log Service Bill</h3>
 <form onSubmit={handleSaveBill} className="space-y-5">
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Bill Date *</label><input type="date" value={billDate} onChange={e => setBillDate(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] text-white focus:border-[#FF5A00] outline-none font-bold" required /></div>
 <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Select Truck *</label><select value={wsTruckId} onChange={e => setWsTruckId(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] text-white focus:border-[#FF5A00] outline-none font-bold" required><option value="">-- SELECT TRUCK --</option>{vehicles.map(v => <option key={v.id} value={String(v.id)}>{v.vehicle_number}</option>)}</select></div>
 </div>
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Vendor / Workshop Name *</label><input type="text" maxLength={60} value={vendor} onChange={e => setVendor(e.target.value.toUpperCase())} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] text-white focus:border-[#FF5A00] outline-none font-bold uppercase" required /></div>
 <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Total Bill Amount () *</label><input type="number" min="1" max="1000000" value={amount} onChange={e => setAmount(e.target.value === "" ? "" : parseFloat(e.target.value))} className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] focus:border-[#FF5A00] outline-none font-black text-rose-500" required /></div>
 </div>
 <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Parts & Service Description</label><input type="text" maxLength={150} value={description} onChange={e => setDescription(e.target.value.toUpperCase())} placeholder="e.g. Engine oil change, 2 brake pads" className="w-full text-sm p-3 rounded-xl border border-[#272B36] bg-[#0F1117] text-white focus:border-[#FF5A00] outline-none font-semibold uppercase" /></div>
 <button type="submit" disabled={isProcessing} className="w-full py-3.5 bg-[#FF5A00] hover:bg-[#e04f00] text-white font-black text-sm rounded-xl transition-all shadow-lg active:scale-95 disabled:bg-slate-700">Save Service Record</button>
 </form>
 ...
 <div className="overflow-x-auto w-full max-h-[400px]">
 <table className="min-w-full divide-y divide-[#272B36] text-xs text-left whitespace-nowrap">
 <thead className="bg-[#0F1117] sticky top-0 z-10"><tr className="font-bold text-slate-400 uppercase tracking-wider text-[10px]"><th className="px-5 py-3.5">Date</th><th className="px-5 py-3.5">Truck</th><th className="px-5 py-3.5">Vendor & Details</th><th className="px-5 py-3.5 text-right">Amount ()</th></tr></thead>
 <tbody className="divide-y divide-[#272B36] bg-[#161922]">
 {filteredBills.map(b => (
 <tr key={b.bill_id} className="hover:bg-[#1E222D]">
 <td className="px-5 py-4 font-semibold text-slate-300">{formatDate(b.bill_date)}</td>
 <td className="px-5 py-4 font-black text-white">{b.trucks?.vehicle_number || "UNKNOWN"}</td>
 <td className="px-5 py-4 text-slate-300 font-bold">{b.vendor_name} <br/><span className="text-[10px] text-slate-500 font-normal">{b.service_description}</span></td>
 <td className="px-5 py-4 text-right font-black text-rose-500">{(b.bill_amount || 0).toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
 </tr>
 ))}
 {filteredBills.length === 0 && <tr><td colSpan={4} className="p-8 text-center text-slate-500 font-medium">No service bills match your search.</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
