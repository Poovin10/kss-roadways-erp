...
export function FinancialsModule() {
 ...
 <div><h2 className="text-xl font-semibold text-white  tracking-tight">Fleet Analytics & Margins</h2><p className="text-xs text-white/60 mt-0.5">High-level financial benchmarks and performance tracking.</p></div>
 </div>

 <div className="bg-[#161922] border border-[#272B36] rounded-2xl p-6 sm:p-8 shadow-xl">
 <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
 <div className="md:col-span-2"><label className="block text-[10px] font-bold text-white/60  mb-1">Analysis Window</label><select value={analysisWindow} onChange={e => setAnalysisWindow(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#2B3142] bg-[#0F1117] text-white outline-none focus:border-[#FF5A00] font-bold"><option value="Current Fiscal Month">Current Fiscal Month</option><option value="Lifetime Fleet">Lifetime Fleet</option><option value="Custom Dates">Custom Dates</option></select></div>
 {analysisWindow === "Custom Dates" && (<><div><label className="block text-[10px] font-bold text-white/60  mb-1">From Date</label><input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#2B3142] bg-[#0F1117] text-white outline-none focus:border-[#FF5A00]" /></div><div><label className="block text-[10px] font-bold text-white/60  mb-1">To Date</label><input type="date" value={customEnd} onChange={e => setCustomEnd(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#2B3142] bg-[#0F1117] text-white outline-none focus:border-[#FF5A00]" /></div></>)}
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-b border-[#272B36] pb-6 mb-6">
 <div><label className="block text-[10px] font-bold text-white/60  mb-1">Sort By Metric</label><select value={sortMetric} onChange={e => setSortMetric(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#2B3142] bg-[#0F1117] text-white outline-none focus:border-[#FF5A00] font-semibold"><option value="Total Net Retention (INR)">Total Net Retention (INR)</option><option value="Total Freight Revenue (INR)">Total Freight Revenue (INR)</option><option value="Total Trips">Total Trips</option><option value="Incomplete Trips">Incomplete Trips</option><option value="Total Tons (MT)">Total Tons (MT)</option><option value="Total Diesel (L)">Total Diesel (L)</option><option value="Total Diesel Expense (INR)">Total Diesel Expense (INR)</option><option value="Retention %">Retention %</option><option value="Diesel %">Diesel %</option><option value="KMPL">KMPL</option></select></div>
 <div><label className="block text-[10px] font-bold text-white/60  mb-1">Sort Order</label><select value={sortOrder} onChange={e => setSortOrder(e.target.value)} className="w-full text-sm p-3 rounded-xl border border-[#2B3142] bg-[#0F1117] text-white outline-none focus:border-[#FF5A00] font-semibold"><option value="Top Performers (Descending)">Top Performers (Descending)</option><option value="Underperformers (Ascending)">Underperformers (Ascending)</option></select></div>
 </div>
 ...
 <button onClick={exportAnalyticsToCSV} className="px-6 py-2.5 bg-[#0F1117] hover:bg-[#1A1F2C] border border-[#272B36] text-emerald-400 font-bold text-sm rounded-xl transition-all shadow-sm flex items-center gap-2 active:scale-95 ">EXPORT CSV</button>
 </div>

 {analyticsSubTab !== "Driver Scorecard" && (
 <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6">
 <div className="p-4 rounded-xl border border-[#2B3142] bg-[#1A1F2C] min-w-0"><p className="text-[9px] sm:text-[10px] font-bold text-white/60  mb-1 truncate">Fleet Revenue</p><p style={{ fontSize: 'clamp(1.1rem, 2vw, 1.875rem)' }} className="font-semibold text-white leading-none whitespace-nowrap">{formatAmt(aggFreight)}</p></div>
 <div className="p-4 rounded-xl border border-rose-900/50 bg-rose-950/20 min-w-0"><p className="text-[9px] sm:text-[10px] font-bold text-rose-500  mb-1 truncate">Diesel Cost</p><p style={{ fontSize: 'clamp(1.1rem, 2vw, 1.875rem)' }} className="font-semibold text-rose-400 leading-none whitespace-nowrap">{formatAmt(aggDiesel)}</p></div>
 <div className="p-4 rounded-xl border border-emerald-900/50 bg-emerald-950/20 min-w-0"><p className="text-[9px] sm:text-[10px] font-bold text-emerald-500  mb-1 truncate">Net Margin</p><p style={{ fontSize: 'clamp(1.1rem, 2vw, 1.875rem)' }} className="font-semibold text-emerald-400 leading-none whitespace-nowrap">{formatAmt(aggRetention)}</p></div>
 <div className="p-4 rounded-xl border border-[#FF5A00]/30 bg-[#FF5A00]/10 min-w-0"><p className="text-[9px] sm:text-[10px] font-bold text-[#FF5A00]  mb-1 truncate">Retention %</p><p style={{ fontSize: 'clamp(1.1rem, 2vw, 1.875rem)' }} className="font-semibold text-[#FF5A00] leading-none whitespace-nowrap">{formatDec(aggRetentionPct)}%</p></div>
 </div>
 )}
 ...
 {sortedFleetData.map((row: any) => (<tr key={row.vehicle_number} className="hover:bg-[#1E222D]"><td className="px-4 py-3 text-left font-semibold text-white">{row.vehicle_number}</td><td className="px-4 py-3 text-left font-bold text-white/60">{row.truck_type}</td><td className="px-4 py-3 font-semibold text-white">{row.total_trips}</td><td className="px-4 py-3 font-semibold text-white/60">{formatDec(row.total_tons)}</td><td className="px-4 py-3 text-rose-500 font-bold">{row.incomplete_trips}</td><td className="px-4 py-3 font-bold text-slate-300">{formatAmt(row.total_freight)}</td><td className="px-4 py-3 font-bold text-white/60">{formatDec(row.total_diesel_litres)}</td><td className="px-4 py-3 font-bold text-rose-400">{formatAmt(row.total_diesel_cost)}</td><td className="px-4 py-3 font-semibold text-[#FF5A00]">{formatAmt(row.net_retention)}</td><td className="px-4 py-3 font-semibold text-emerald-400">{formatDec(row.retention_pct)}%</td><td className="px-4 py-3 font-bold text-white/60">{formatDec(row.diesel_pct)}%</td><td className="px-4 py-3 font-bold text-amber-500">{formatDec(row.kmpl)}</td></tr>))}
 {sortedFleetData.length === 0 && !isAnalyticsLoading && (<tr><td colSpan={12} className="px-4 py-8 text-center text-white/40 font-medium">No fleet data found for this period.</td></tr>)}
 </tbody>
 </table>
 )}
 ...
 <tbody className="bg-[#161922] divide-y divide-[#272B36]">
 {driverScorecard.map((row: any) => (<tr key={row.driver_code} className="hover:bg-[#1E222D]"><td className="px-6 py-4 text-left font-semibold text-white">{row.driver_code}</td><td className="px-6 py-4 text-left font-bold text-slate-300">{row.full_name}</td><td className="px-6 py-4 font-semibold text-[#FF5A00]">{row.trips}</td><td className="px-6 py-4 font-semibold text-white/60">{formatDec(row.total_km)}</td><td className="px-6 py-4 font-semibold text-amber-500">{formatDec(row.kmpl)}</td><td className="px-6 py-4 font-semibold text-emerald-400">{formatAmt(row.revenue)}</td></tr>))}
 {driverScorecard.length === 0 && !isAnalyticsLoading && (<tr><td colSpan={6} className="px-6 py-8 text-center text-white/40 font-medium">No driver activity logged in this period.</td></tr>)}
 </tbody>
 </table>
 )}
 </div>
